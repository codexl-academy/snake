...
El día anterior definimos una tipo de dato enumerable en Python de la siguiente forma:
...
Afortunadamente tenemos **la herencia**. Es evidente que una `PersonaConEmpleo` es una caso particular de una `Persona`. Es decir, es una persona que **además** tiene otros atributos y/o comportamientos. Cuando esto sucede, decimos que `PersonaConEmpleo` **hereda** de `Persona`. Y en Python podemos expresarlo de la siguiente manera:
...
Es por eso que para crear nuestra clase `Direction` necesitamos heredar de la clase `Enum` y especificar los campos específicos que queremos incluir.
...
De aquí es de donde viene la variable `i`. En Python podemos definir collecciones ordenadas de elementos (en este caso tuplas de dos enteros) usando la técnica que se conoce como *list comprehension*. El código anterior se puede entender mejor si comenzamos a leerlo a partir de la palabra `for`.
...
>**Nota :pen:** La función `range` nos devuelve una colección oredenada del rango de número que determinan sus argumentos. Por ejemplo, `range(0, 3)` devuelve la colección `[0, 1, 2]`. O sea todos los números enteros entre el primer parámetro y el segundo, excluyendo el segundo.

Para ilustrar mejor, tomemos `position = (30, 2)` y `size = 3`. ¿Qué colección nos devuelve el código anterior? En primer lugar recordemos que inicialmente `body = [position]`, o sea, `body = [(10,2)]` en nuestro ejemplo. La variable `i` toma los valores sucesivos `1, 2` debido a que `size = 3`. Por tanto:
...
Como `body` es una lista, tiene todas las funciones que Python tiene predefinidas para sus listas. Una de esas funciones es `extend`. La función `extend` recibe una colección y la concatena con la lista actual. Siguiendo el ejemplo anterior, si tenemos inicialmente `body = [(30, 2)]` y hacemos `body.extend([(20, 2), (10, 2)])`, la lista `body` se aumenta de tamaño y se convierte en la lista `[(30, 2), (20, 2), (10, 2)]`. De esta forma tenemos en la lista `body` la posición de cada parte del cuerpo de la serpiente. Lo último que tenemos que hacer es retornar esta lista y así completamos la función `_create_body`
...
Esta función recibe dos direcciónes: `direction1` y `direction2`. Y determina que no sean totalmente opuestas. Cada sentencia entre paréntesis es una condición. Por ejemplo la primera:
...
Y bueno, creemos que por hoy es suficiente :party:. Se hacía necesario entender bien todo el código que habíamos hecho ayer y esperamos que lo hayas logrado. Si no es así lee nuevamente esta lección y la anterior para que te enteres de todo. ¡No hay prisa! En la siguiente lección te introduciremos en el mundo del *testing*. ¿Cómo hacer programas que comprueben que todo lo que hemos hecho funciona correctamente? Ya lo verás. Esta es una de las habilidades más demandadas en la industria de hoy en día y te lo mostraremos todo de forma práctica mañana.
